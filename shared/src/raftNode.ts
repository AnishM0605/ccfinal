...
for (const incoming of payload.entries) {
  ...
  if (this.log.length === 0) {
    // Fresh node recovery
    this.logger.warn("Fresh follower — rebuilding log from leader");
    ...
    try {
      ...
      this.logger.warn("Repairing log gap", {
        expected: this.log.lastIndex + 1,
        received: incoming.index
      });

      this.log.truncateTo(this.log.lastIndex);
  ...
  this.logger.warn("Majority not reached yet (eventual consistency)", {
    index: entry.index,
    commitIndex: this.commitIndex
  });
